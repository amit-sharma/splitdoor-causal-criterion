correlational_estimate <- function(tseries_df,
  ...
  if(is.null(test_time_period)){
    ctr_list= est_naive_nodatefactors(tseries_tbl) 
    ctr_xy_naive = ctr_list$ctr_iv_data_xy
    ctr_x_naive = ctr_list$ctr_iv_data_x
  } else {
    aug_tseries_tbl = fn_add_time_period_factor(tseries_tbl, test_time_period)
    ctr_list = est_naive(aug_tseries_tbl)
    ctr_xy_naive = ctr_list$ctr_iv_data_xy
    ctr_x_naive = ctr_list$ctr_iv_data_x  
  }
...
splitdoor_causal_estimate <- function(tseries_df, 
  ...
  tseries_tbl <- as.tbl(tseries_df)  
  distinct_ids = select(tseries_tbl,asin_id) %>%
    distinct()
  ...
  #                                  date_factor, date, asin_id, target_asin_id,
  #                                  x_visits, ytotal_visits, discrete_x,
  #                                  num_clickthroughs, yd_visits, discrete_yd, 
  #                                  is_constant_demand, num_demand_levels)
  # 
  by_pgroup = FALSE
  draw_plots = FALSE
  
  indep_treatment_outcome_pairs = fn_independence_test(aug_tseries_tbl,
                                    p_value=independence_threshold,
                                    num_discrete_levels=num_discrete_levels)
  tseries_tbl_with_indep_info = left_join(aug_tseries_tbl, indep_treatment_outcome_pairs, 
  ...
  valid_tseries_tbl = filter(tseries_tbl_with_indep_info, !is.na(num_values))#!is.na(num_diff_yds))  
  notvalid_tseries_tbl = filter(tseries_tbl_with_indep_info, is.na(num_values))
#}
# return(list(
...
#    dep_asin_targets_visits =  notvalid_tseries_tbl
  ...
  if(draw_plots){
    #indep_unique_asins = (indep_asin_targets_visits, asin_id) %>% distinct()
    # plot for paper
    ...
    ctr_df_list = est_causal_effect(valid_tseries_tbl,
                                    method="simple", # regression_cov, simple
                                    check_all=FALSE)
    ctr_xy_indep = ctr_df_list$ctr_xy
    ctr_x_indep = ctr_df_list$ctr_x
    ...
    #tt=est_independent_x_yd_pearl(filter(indep_asin_targets_visits, asin_id %in% ctr_x_indep$asin_id[1:3000]), 
...
compare_splitdoor_correlational_estimate <-function(splitdoor_estimates_df, correlational_estimates_df, by_group=TRUE){
  tseries_with_valid_splitdoor = unique(splitdoor_estimates_df$asin_id)
  ctr_corr_tseries_with_valid_splitdoor = filter(correlational_estimates_df, asin_id %in% tseries_with_valid_splitdoor)
  if(by_pgroup){
